...
OUT = ROOT + '/apps/client/public/models/roster'
os.makedirs(OUT, exist_ok=True)
os.makedirs(ROOT + '/assets/characters', exist_ok=True)
scene = bpy.data.scenes.new('Fantasy Arena Expanded Roster')
...
manifest = {'version': 2, 'units': 'meters', 'forward': '+Z', 'models': {}}
class Model:
    ...
    def shape(self, verts, faces, color):
        ...
        for i,f in enumerate(faces):
            self.faces.append([start+n for n in f]); self.colors.append(tuple(min(.9999,v*(.93+(i%3)*.035)) for v in rgb(color)))
    def box(self,p,s,c):
    ...
    def beam(self,a,b,r,c):
        a=Vector(a); b=Vector(b); direction=(b-a).normalized()
        tangent=direction.cross(Vector((1,0,0)))
        if tangent.length<.01: tangent=direction.cross(Vector((0,0,1)))
        tangent.normalize(); other=direction.cross(tangent)
        verts=[tuple(p+r*(math.cos(i*math.pi/2)*tangent+math.sin(i*math.pi/2)*other)) for p in [a,b] for i in range(4)]
        self.shape(verts,[(0,3,2,1),(4,5,6,7)]+[(i,(i+1)%4,(i+1)%4+4,i+4) for i in range(4)],c)
    def finish(self):
        ...
        for poly,c in zip(mesh.polygons,self.colors):
            for index in poly.loop_indices: col.data[index].color=(*c,.9999)
    ...
    def export(self):
        with bpy.context.temp_override(scene=scene, view_layer=scene.view_layers[0]):
            ...
            bpy.ops.export_scene.gltf(filepath=OUT+'/'+self.name+'.glb',export_format='GLB',use_selection=True,export_apply=True,export_extras=True,use_active_scene=True,export_animations=False)
            manifest['models'][self.name]={'url':'/models/roster/'+self.name+'.glb','parts':self.parts}
...
def humanoid(name):
    dwarf=name=='guardian'; ranged=name in ['archer','mage']; goblin=name in ['goblin','runner','brute','revenant','boss']
    m=Model(name)
    metal='#b7c9d0' if not dwarf else '#a88652'
    cloth='#426c91' if not dwarf else '#66503c'
    skin={'goblin':'#70954b','runner':'#d88041','brute':'#8e839d','revenant':'#7dc7d8','boss':'#a95742'}.get(name,'#e0ad80')
    if ranged: metal='#658054' if name=='archer' else '#7868a9'; cloth='#395438' if name=='archer' else '#423a72'
    ...
    width=.7 if dwarf else .57 if not goblin else .72 if name=='brute' else .65 if name=='boss' else .38 if name=='runner' else .44
    ...
    if ranged:
        # Open faces, hair and a cloth hood leave each class readable.
        for side in [-1,1]:
            m.box((side*.10,head+.025,.249),(.065,.065,.03),dark)
        m.box((0,head-.11,.25),(.10,.025,.025),'#8a5243')
        if name=='archer':
            m.prism((0,head+.23,-.04),(.31,.29),.22,cloth,6,top=.65)
            m.box((.24,head+.43,-.05),(.045,.32,.06),'#e6bd69')
        else:
            # Brim and lower crown belong to the head; only the upper tip flexes.
            m.prism((0,head+.23,0),(.46,.40),.08,cloth,8,top=1)
            m.prism((0,head+.355,0),(.29,.26),.25,metal,7,top=.54)
            m.prism((0,head+.28,0),(.29,.265),.09,gold,7,top=.95)
            m.box((0,head+.31,.26),(.11,.12,.04),'#80e1ef')
    elif not goblin:
    ...
    if goblin:
        # Distinct silhouettes beyond palette swaps; a loose belt talisman on every enemy.
        if name=='runner':
            for j in range(3): m.prism((0,head+.38+j*.08,-.10-j*.09),(.07,.10),.30,'#ffd078',4,top=0)
        if name in ['brute','boss']:
            for side in [-1,1]:
                m.prism((side*(width*.5+.15),chest+.28,0),(.25,.28),.22,'#525566',5,top=.85)
                m.prism((side*(width*.5+.15),chest+.49,0),(.09,.09),.29,'#dfd8bd',4,top=0)
        if name=='boss':
            for x in [-.22,0,.22]: m.prism((x,head+.43,0),(.075,.08),.38,gold,4,top=0)
        if name=='revenant':
            for side in [-1,1]: m.prism((side*.23,chest+.40,-.08),(.13,.12),.48,'#c2f4f3',4,top=0)
    ...
    if dwarf:
        m.box((handx,handy,.60),(.10,.11,1.05),leather)
        m.box((handx,handy,1.04),(.14,.15,.15),gold)
        for side in [-1,1]:
            m.shape([(handx+side*.05,handy-.065,.83),(handx+side*.05,handy-.065,1.20),(handx+side*.44,handy-.04,1.30),(handx+side*.52,handy-.04,.86),(handx+side*.05,handy+.065,.83),(handx+side*.05,handy+.065,1.20),(handx+side*.44,handy+.04,1.30),(handx+side*.52,handy+.04,.86)],[(0,1,2,3),(4,7,6,5),(0,4,5,1),(1,5,6,2),(2,6,7,3),(3,7,4,0)],'#cbd9dc')
    elif name=='archer':
        # Curved stave in the vertical plane; separate from the hand and quiver.
        points=[(handx,handy+y,.25+.32*(1-(y/.65)**2)) for y in [-.65,-.45,-.22,0,.22,.45,.65]]
        for a,b in zip(points,points[1:]): m.beam(a,b,.065,'#b7864d')
        m.beam(points[0],points[-1],.014,'#e1d7b4')
        m.box((handx,handy,.55),(.095,.20,.09),leather)
    elif name=='mage':
        m.prism((handx,handy+.25,.2),(.055,.055),1.65,'#8c6844',6,top=.8)
        m.prism((handx,handy+1.08,.2),(.15,.15),.16,gold,6,top=1.2)
        m.prism((handx,handy+1.28,.2),(.14,.14),.33,'#80e1ef',5,top=0)
        m.prism((handx,handy+1.06,.2),(.14,.14),.12,'#897be0',5,top=1)
    else:
    ...
    if dwarf:
        ...
        m.part('quiver',(0,chest-.08,-.34),(.16,.32,.14),'torso',(0,chest+.19,-.30),role='accessory')
        m.prism((0,chest-.08,-.34),(.18,.16),.64,leather,8,top=1.12)
        for y in [chest-.34,chest+.20]: m.prism((0,y,-.34),(.195,.175),.055,gold,8,top=1)
        for x in [-.10,0,.10]:
            m.beam((x,chest+.12,-.34),(x,chest+.53,-.34),.025,'#bd955e')
            m.box((x,chest+.47,-.34),(.075,.13,.025),'#eee0b4')
    elif name=='mage':
        m.part('hat_tip',(0,head+.60,0),(.16,.15,.15),'head',(0,head+.48,0),role='accessory')
        m.prism((0,head+.60,0),(.157,.141),.25,metal,7,top=.15)
        m.beam((0,head+.68,0),(.17,head+.75,.02),.08,metal)
    elif not goblin:
    ...
    if goblin:
        m.part('talisman',(0,hip-.16,.31),(.08,.13,.05),'pelvis',(0,hip+.02,.30),role='accessory')
        m.beam((0,hip+.02,.30),(0,hip-.18,.31),.025,leather)
        m.prism((0,hip-.20,.31),(.075,.05),.12,gold if name=='boss' else '#ded8b5',5,top=.4)
    return m.export()

models=[humanoid(name) for name in ['warrior','guardian','archer','mage','goblin','runner','brute','revenant','boss']]
def projectile(name):
    m=Model(name); m.part('projectile',(0,0,0),(.10,.10,.40),role='projectile')
    if name=='arrow':
        m.beam((0,0,-.4),(0,0,.28),.022,'#b9925d')
        m.shape([(-.09,0,.22),(.09,0,.22),(0,0,.44),(0,.035,.22)],[(0,1,2),(0,3,1),(0,2,3),(1,3,2)],'#d5e2e4')
        m.box((0,0,-.3),(.17,.018,.14),'#e8dabc')
        m.box((0,0,-.3),(.018,.17,.14),'#e8dabc')
    else:
        m.prism((0,0,0),(.13,.20),.22,'#8cefff',6,top=.5)
        m.shape([(-.10,0,0),(.10,0,0),(0,.10,0),(0,-.10,0),(0,0,-.48)],[(0,2,4),(2,1,4),(1,3,4),(3,0,4)],'#8e78ec')
    return m.export()
projectile_models=[projectile(name) for name in ['arrow','magic_missile']]
with open(ROOT+'/apps/client/public/models/manifest.json','w') as f: json.dump(manifest,f,indent=2)
for i,m in enumerate(models):
    m.rig.location.x=(i%5-2)*2.1; m.rig.location.y=0 if i<4 else 3.4
for i,m in enumerate(projectile_models): m.rig.location=(i*1.5-1,-1.8,.5)
...
camera.location=(7,-13,10); camera.rotation_euler=(Vector((0,1.4,1))-camera.location).to_track_quat('-Z','Y').to_euler(); data.type='ORTHO'; data.ortho_scale=12.5; scene.camera=camera
bpy.ops.wm.save_as_mainfile(filepath=ROOT+'/assets/characters/fantasy-arena-roster.blend')
scene.render.filepath=ROOT+'/assets/characters/roster-lineup.png'
print('Exported 4 heroes, 5 enemies, 2 projectiles and saved fantasy-arena-roster.blend.')
