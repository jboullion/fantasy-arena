...
import type { Actor, GameEvent, Player } from './index';
...
export type WorldSnapshot = { players: Player[]; enemies: Actor[]; time: number; kills: number; phase: 'playing' | 'paused' | 'dead' | 'complete'; config: Config; events: GameEvent[]; round: number };
